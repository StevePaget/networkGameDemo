...
def drawText(text, x, y, size, colour, font="Arial"):
    fontFace = pygame.font.match_font(font)
    font = pygame.font.Font(fontFace, size)
    fontColour = parseColour(colour)
    screen.blit(font.render(text, True, fontColour),(x,y))
    if screenRefresh:
        updateDisplay()
...
def moveToFront(sprite):
    spriteGroup.move_to_front(sprite)
    
def save(filename, width, height):
    textRects = textboxGroup.draw(screen)
    saveimage = pygame.Surface((width,height))
    pygame.display.update()
    saveimage.blit(screen,(0,0),(50,75))
    pygame.image.save(saveimage, filename)
    textboxGroup.clear(screen, background.surface)
